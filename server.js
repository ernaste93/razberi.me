...
ЕЗИК – ЗАДЪЛЖИТЕЛНО:
- Пиши САМО на чист книжовен български с кирилица. НИКОГА не пиши на латиница ("ne moje" → "не може", "hora" → "хора"). Шльокавица е забранена абсолютно.
- САМО ЧИСТ КНИЖОВЕН БЪЛГАРСКИ: Платформата обучава деца да пишат правилно. Ако дадена дума звучи като руска, сръбска или е чужда калка – замени я с коректната българска. Примери: "называт" → "наричат", "данный" → "този", "являться" → "е/представлява", "получается" → "оказва се". Когато се колебаеш между две форми, избирай по-родната, по-простата, по-чистата българска дума. Нито една дума в отговора ти не трябва да може да бъде разпозната като заемка от друг език, освен ако самата тема не я изисква.
...
async function handleExplain(req, res) {
  ...
  try {
    ...
    logZnaykoMessage(userToken, payload, text).catch(() => {});
...
async function logZnaykoMessage(userToken, payload, assistantText) {
  if (!userToken) return;
  const userId = getUserIdFromJwt(userToken);
  if (!userId) return;

  const lessonSlug = payload.lessonSlug || null;
  const userText = payload.selectedText || payload.studentAnswer || '';

  // Намери или създай сесия за тази lesson за днес
  const today = new Date().toISOString().slice(0, 10);
  let sessionId;

  const sessResp = await fetch(
    `${SUPABASE_URL}/rest/v1/znayko_sessions?user_id=eq.${userId}&lesson_slug=eq.${encodeURIComponent(lessonSlug || '')}&started_at=gte.${today}T00:00:00Z&order=started_at.desc&limit=1`,
  ...
  if (sessions.length > 0) {
    ...
    const createResp = await fetch(`${SUPABASE_URL}/rest/v1/znayko_sessions`, {
      method: 'POST',
      headers: {
    ...
    const created = createResp.ok ? await createResp.json() : [];
  ...
  const msgs = [];
  if (userText) msgs.push({ session_id: sessionId, role: 'user', content: userText });
  msgs.push({ session_id: sessionId, role: 'assistant', content: assistantText });

  await fetch(`${SUPABASE_URL}/rest/v1/znayko_messages`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_SERVICE_KEY,
      Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
